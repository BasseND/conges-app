...
@php
    ...
    $config = $roleConfig[$role] ?? ['bg' => 'from-gray-500 to-gray-600', 'icon' => 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'];
@endphp

<div {{ $attributes->merge(['class' => 'inline-flex items-center px-3 py-1.5 rounded-xl bg-gradient-to-r ' . $config['bg'] . ' text-white shadow-lg']) }}>
    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $config['icon'] }}"/>
    </svg>
    <span class="text-xs font-semibold">{{ \App\Models\User::getRoles()[$role] ?? ucfirst($role) }}</span>
</div>