<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold pb-5 text-bgray-900 dark:text-white">
            {{ __('Ajouter un paramètre de paie') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('admin.payroll-settings.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="max-w-xl">
                            ...
                            <div class="mb-6">
                                ...
                                <select id="type" name="type" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full" required>
                                    ...
                                    <option value="earning" {{ old('type') == 'earning' ? 'selected' : '' }}>Gain</option>
                                    <option value="deduction" {{ old('type') == 'deduction' ? 'selected' : '' }}>Déduction</option>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex">
                                    <x-text-input id="value" class="block mt-1 w-full" type="number" name="value" :value="old('value')" step="0.01" min="0" required />
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                <div class="flex items-center">
                                    <input id="is_percentage" name="is_percentage" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" {{ old('is_percentage') ? 'checked' : '' }}>
                            ...
                            <div class="mb-6">
                                ...
                                <select id="applies_to" name="applies_to" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full" required>
                                    ...
                                    <option value="all" {{ old('applies_to') == 'all' ? 'selected' : '' }}>Tous les employés</option>
                                    <option value="department" {{ old('applies_to') == 'department' ? 'selected' : '' }}>Département spécifique</option>
                                    <option value="contract_type" {{ old('applies_to') == 'contract_type' ? 'selected' : '' }}>Type de contrat spécifique</option>
                                </select>
                            </div>

                            <!-- Département (conditionnel) -->
                            <div id="department_section" class="mb-6 {{ old('applies_to') == 'department' ? '' : 'hidden' }}">
                            ...
                            <div id="contract_type_section" class="mb-6 {{ old('applies_to') == 'contract_type' ? '' : 'hidden' }}">
